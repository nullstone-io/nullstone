...
func (p Provider) Push(nsConfig api.Config, details app.Details, userConfig map[string]string) error {
	...
	if !strings.Contains(targetUrl.Registry, "gcr.io") {
		return fmt.Errorf("this app only supports push to GCP Container Registry (image=%s)", targetUrl)
	}
	...
	if ic.Outputs.ImagePusher.KeyFile == "" {
	...
	targetAuth, err := ic.GetGcrLoginAuth()
	if err != nil {
